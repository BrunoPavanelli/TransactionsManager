...
transactions.use((req, res, next) => usersMiddlewares.verifyToken(req, res, next))

transactions.get(
    "/token",
    (req, res) => transactionsController.findByToken(req, res)
)

transactions.use((req, res, next) => usersMiddlewares.verifyAdminRole(req, res, next))

transactions.post(
    "/upload", 
    upload.single("file"), 
    (req, res) => transactionsController.importFile(req, res)
)
